...
export default function CardTarefa({ tarefa, index, aoDeletar, aoAdicionarEtapa, aoDeletarEtapa, aoAlternarStatusEtapa, aoEditar }) {
  ...
  const inputNovaEtapaRef = useRef(null);
  const inputEditarTituloRef = useRef(null);

  useEffect(() => {
    if (mostrandoCaixaEtapa && inputNovaEtapaRef.current) {
      inputNovaEtapaRef.current.focus();
    }
  }, [mostrandoCaixaEtapa]);

  useEffect(() => {
    if (editando && inputEditarTituloRef.current) {
      inputEditarTituloRef.current.focus();
    }
  }, [editando]);
  ...
  return (
    <div className={`card-tarefa ${tarefa.concluida ? 'tarefa-concluida' : ''}`}>
      ...
      <div className="botoes-acao">
        ...
        <span onClick={() => setMostrandoCaixaEtapa(true)} style={{ cursor: 'pointer' }}>🧭</span>
      </div>
      ...
      {editando && (
        <div className="caixa-nova-etapa" style={{ backgroundColor: '#ffeeba', borderColor: '#ffc107' }}>
          ...
          <div className="caixa-body" style={{ flexDirection: 'column' }}>
            <input 
              ref={inputEditarTituloRef} /* 👈 NOVO: Conectando a referência */
              type="text" 
              value={tituloEditado}
